...
double dp(int mask) { // DP state = mask
    ...
    while (m) {
        ...
        ans = min(ans, dis[p1][p2] + dp(mask^two_pow_p1^two_pow_p2));
    ...
    while (scanf("%d", &N), N) { // yes, we can do this :)
        ...
        for (int i = 0; i < 2*N-1; ++i)
            for (int j = i+1; j < 2*N; ++j)
                dist[i][j] = dist[j][i] = hypot(x[i]-x[j], y[i]-y[j]); // use 'hypot' function
        for (int i = 0; i < (1<<16); ++i) memo[i] = -1.0;
        printf("Case %d: %.2lf\n", ++caseNo, dp((1<<(2*N)) - 1));
    }
    return 0;
} // DP to solve min weighted perfect amtching on small general graph
